...
#     inset_px, inset_frac, min_gap_span_px
#
...
# ---------- PNG → grayscale (no external libs) ----------
...
# ---------- core QA with interior ROI + min gap span ----------
def run_png_qa_core(png_bytes, params):
    ...
    if not px_per_in:
        ...
        if not px_per_in:
            if pw and pw > 0: px_per_in = width / pw
            elif ph and ph > 0: px_per_in = height / ph
            else: px_per_in = 300.0

    # Interior ROI to avoid edge nicks
    user_inset_px  = int(max(0, fnum(params.get("inset_px"), 0)))
    inset_frac     = fnum(params.get("inset_frac"), 0.02)   # 2% default
    auto_inset_px  = int(round(min(roi_w, roi_h) * inset_frac)) if roi_w and roi_h else 0
    inset_px       = max(user_inset_px, auto_inset_px)
    roi_interior   = inset_roi(roi, inset_px)

    # AA tolerance & cleaning
    ...
    # Minimum meaningful span for a gap (ignore tiny 1–2 px nicks)
    default_min_span_px = int(max(3, round(px_per_in * 0.006)))  # ≈0.006"
    min_gap_span_px     = int(max(3, fnum(params.get("min_gap_span_px"), default_min_span_px)))
    if pos_gap_closed and pos_gap_closed.get("len_px", 999999) < min_gap_span_px:
    ...
    TH_TEXT = num(params.get("min_text_height_in"),    0.10)      # 0.10"
    TH_GAP  = num(params.get("min_negative_space_in"), 1.0/72.0)  # 1 pt
    TH_LINE = num(params.get("min_line_weight_in"),    0.005)     # 0.005"
...
if __name__ == "__main__":
    # For local runs; your platform (Railway/Render/etc) will set PORT
    import os
