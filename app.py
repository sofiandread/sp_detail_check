...
# NEW: ignore tiny components when estimating text height
def estimate_min_text_height_px_filtered(ink, w, h, roi=None, min_comp_px=3):
    if roi is None:
        minX, minY, maxX, maxY = bbox_of_ink(ink, w, h)
    else:
        minX, minY, maxX, maxY = roi
    ...
    for y in range(minY, maxY + 1):
        for x in range(minX, maxX + 1):
            ...
            while stack:
                ...
                if cx > minX:
                    n = cidx - 1
                    if ink[n] == 1 and lab[n] == 0:
                        lab[n] = label; stack.append((cx - 1, cy))
                if cx < maxX:
                    n = cidx + 1
                    if ink[n] == 1 and lab[n] == 0:
                        lab[n] = label; stack.append((cx + 1, cy))
                if cy > minY:
                    n = cidx - w
                    if ink[n] == 1 and lab[n] == 0:
                        lab[n] = label; stack.append((cx, cy - 1))
                if cy < maxY:
                    n = cidx + w
                    if ink[n] == 1 and lab[n] == 0:
                        lab[n] = label; stack.append((cx, cy + 1))
            ...
            # ignore micro specks / anti-aliased crumbs
...
def run_png_qa_core(png_bytes, params):
    ...
    # anti-alias tolerance & cleaning for negative space
    ...
    # measure using cleaned/closed ink
    min_line_px_raw, _mg_unused, pos_line_raw, _pg_unused = measure_min_line_and_gap_pos(
        ink_clean, width, height, roi
    )
    _ml_closed, min_gap_px_closed, _pl_unused, pos_gap_closed = measure_min_line_and_gap_pos(
        ink_closed, width, height, roi
    )

    # text: estimate on the closed (denoised) ink; ignore tiny components
    text_min_comp_px = int(max(alias_px * 2, fnum(params.get("text_min_comp_px"), 3)))
    min_text_px = estimate_min_text_height_px_filtered(
        ink_closed, width, height, roi, min_comp_px=text_min_comp_px
    )

    # final, robust metrics (clamp minima by alias tolerance)
    ...
    # confidence: min of (measured/threshold) across available metrics, clamped 0..1
    ratios = []
    if metrics["min_text_height_in"]    is not None: ratios.append(metrics["min_text_height_in"]   / TH_TEXT)
    if metrics["min_line_weight_in"]    is not None: ratios.append(metrics["min_line_weight_in"]   / TH_LINE)
    if metrics["min_negative_space_in"] is not None: ratios.append(metrics["min_negative_space_in"]/ TH_GAP)
...
def png_qa():
    try:
        # collect params (JSON or form-data text)
        params = {}
        if request.is_json:
        ...
        # image bytes: multipart file > base64 text
        png_bytes = None
